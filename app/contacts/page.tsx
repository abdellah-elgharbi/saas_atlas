...
export default function Contacts() {
  ...
  if (isLimitLoading || loadingData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-primary/70 dark:border-primary-light"></div>
        <p className="mt-4 text-slate-500 dark:text-slate-400 text-sm">Loading contacts…</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Header */}
      <div className="flex justify-between items-center flex-shrink-0 animate-slide-in">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-3">
            Contacts Directory
            {isCachedMode && (
              <span className="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800 animate-scale-in">
                Cached Mode
              </span>
            )}
          </h2>
          <p className="text-slate-600 dark:text-slate-400 mt-1">
            {isCachedMode 
              ? "Displaying contacts already viewed today." 
              : "Search and connect with educational administrators."}
          </p>
        </div>

        <div className={`px-4 py-2 rounded-xl shadow-sm text-sm font-medium border transition-all duration-300 hover:scale-105 ${
          limitReached 
      ...
      {/* Cached mode info */}
      {isCachedMode && (
        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 shadow-sm flex items-start gap-4 animate-slide-in hover-lift">
          <Database className="w-6 h-6 text-blue-600 dark:text-blue-300 flex-shrink-0 animate-pulse" />
          <div>
            <h4 className="font-semibold text-blue-900 dark:text-blue-100 text-sm">Cached Data</h4>
            <p className="text-blue-700 dark:text-blue-300 text-sm mt-1">
              You have reached your daily limit. Only previously viewed contacts are visible.
            </p>
          </div>
        </div>
      )}
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {displayContacts.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-6 py-8 text-center text-slate-500 dark:text-slate-400">No contacts available.</td>
                </tr>
              ) : (
                displayContacts.map((contact) => {
                  return (
                    <tr
                      key={contact.id}
                      className="hover:bg-blue-50 dark:hover:bg-slate-800/40 transition-all cursor-pointer hover:shadow-md transform hover:scale-[1.01]"
      ...
      {!isCachedMode && totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 flex-wrap">
          <button
            ...
            disabled={page === 1}
            className="px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-white disabled:opacity-40 disabled:cursor-not-allowed hover:bg-slate-100 dark:hover:bg-slate-800 transition-all hover:scale-105 active:scale-95"
          >
            ← Previous
          </button>
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg border transition-all select-none ${
                p === page
                  ...
                  : 'border-slate-300 dark:border-slate-700 text-slate-700 dark:text-white hover:bg-slate-100 dark:hover:bg-slate-800 hover:scale-105'
              } ${p === '...' ? 'opacity-60 cursor-default pointer-events-none' : 'active:scale-95'}`}
            >
              {p}
            </button>
          ))}
      ...
      {selectedContact && (
        ...
        >
          <div
            className="max-w-2xl w-full max-h-[85vh] overflow-hidden bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl flex flex-col animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex justify-between items-start p-6 border-b border-slate-200 dark:border-slate-700 flex-shrink-0">
              ...
              <button
                onClick={() => setSelectedContact(null)}
                className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 text-2xl flex-shrink-0 transition-all hover:rotate-90 hover:scale-110 rounded-full p-1"
              >
                ×
              </button>
            </div>
